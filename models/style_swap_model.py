...
class StyleSwapModel(BaseModel):
    ...
    def _build_train_model(self):
        preprocess_fn = preprocessing_factory.get_preprocessing(
            self.config.net_name, is_training=False)
        [image] = self.records_loader.get_data()
        preprocessed_image = preprocess_fn(
            image, self.PREPROCESS_SIZE, self.PREPROCESS_SIZE)
        images = self.records_loader.batch_data(preprocessed_image)

        [style_image] = self.style_loader.get_data()
        preprocessed_style_image = preprocess_fn(
            style_image, self.PREPROCESS_SIZE, self.PREPROCESS_SIZE)
    ...
    def _build_evaluate_model(self):
        ...
        preprocessed_image = preprocess_fn(
            self.input_image, height, width, resize_side_min=min(height, width))
        images = tf.expand_dims(preprocessed_image, axis=0)

        style_images = tf.expand_dims(preprocess_fn(
            self.style_image, self.PREPROCESS_SIZE, self.PREPROCESS_SIZE), axis=0)
    ...
    def _swap_net(self, content, style):
        network_fn = nets_factory.get_network_fn(
            self.config.net_name, num_classes=1, is_training=False)
        ...
        content_feature = endpoints_dict[self.config.net_name +
                                         self.style_layer]
        ...
        with tf.variable_scope("", reuse=True):
            ...
            [layer_name] = [
                l_name for l_name in layer_names if self.style_layer in l_name]
    ...
    def _train_inverse(self, generated, swaped_tensor):
        ...
        with tf.variable_scope("", reuse=True):
            ...
            _, inversed_endpoints_dict = network_fn(
                preprocessed_image, spatial_squeeze=False)
        ...
        ) if var.name.startswith("inverse_net")]
        ...
        self.train_op = tf.train.AdamOptimizer(learning_rate).minimize(
            self.loss_op, self.global_step, train_vars)
    ...
    def _swap_op(self, content_feature, style_feature):
        ...
        similarity = tf.nn.conv2d(content_feature, normalized_filters, strides=[
                                  1, 1, 1, 1], padding="VALID")

        arg_max_filter = tf.argmax(similarity, axis=-1)
        one_hot_filter = tf.one_hot(
            arg_max_filter, depth=similarity.get_shape()[-1].value)
